...
<script>
	...
	<?php }else{ ?>
		...
		function generateQuestion(difficulty, type){
			generateEngOperation(difficulty, type)
		...
		function generateProblem(difficulty, type, number, dragOrInput) {
			...
			QuestionAnswers = QuestionData[number];
			problemBox.className = "problem-box";
			questionNo.innerHTML = "Problem " + number;
			label.innerHTML = "Your Answer: ";
			correctAnswers[number - 1] = QuestionAnswers[0];
			if(dragOrInput == 0) { //drag and drop style
				if(type == "sy")
					question.innerHTML = "Choose The Correct Synonym For <b>" + QuestionAnswers[3] + "</b>";
				else 
					question.innerHTML = "Choose The Correct Spelling";
				...
				if(type == "sy")
					question.innerHTML = "Input Correct Synonym Of <b>" + QuestionAnswers[3] + "</b>";
				else 
					question.innerHTML = "Input Correct Spelling Of <b>" + QuestionAnswers[2] + "</b>";
		...
		function generateEngOperation(difficulty, type) {
			var operation;
			var correctAnswer;
			var badAndwer1;
			var badAndwer2;
			$.getJSON( "get_question.php?difficulty=" + difficulty +"&type=" + type + "&limit=" + totalQuestions, function(data) {
			  for(var i=0;i<data.length;i++){
				  QuestionData[i] = []
				  QuestionData[i][0] = data[i]['correctAnswer']
				  QuestionData[i][1] = data[i]['badAndwer1']
				  QuestionData[i][2] = data[i]['badAndwer2']
				  QuestionData[i][3] = data[i]['word']
			  }
					var dragOrInput = getRandomArbitrary(0, 2);
					generateProblem(difficulty, type, QuestionCount, dragOrInput);
				
			})
			.fail(function() {
			  alert( "error" );
			})	
			return [operation, correctAnswer, badAndwer1, badAndwer2];
		}
